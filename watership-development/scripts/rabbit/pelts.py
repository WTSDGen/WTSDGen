...
class Pelt():
    ...
    # scars1 is scars from other rabbits, other animals - scars2 is missing parts - scars3 is "special" scars that could only happen in a special event
    ...
    pelt_rabbitegories = [tabbies, spotted, plain, exotic, torties]
    ...
    colour_rabbitegories = [ginger_colours, black_colours, white_colours, brown_colours]
    eye_sprites = [
    ...
    """Holds all appearence information for a rabbit. """
    def __init__(self,
        ...
        self.rabbit_sprites['newborn'] = 20
        self.rabbit_sprites['para_young'] = 17
        self.rabbit_sprites["sick_adult"] = 18
        self.rabbit_sprites["sick_young"] = 19
    ...
    def init_eyes(self, parents):
        ...
        num = game.config["rabbit_generation"]["base_heterochromia"]
    ...
    def pattern_color_inheritance(self, parents: tuple=(), gender="female"):
        # setting parent pelt rabbitegories
        #We are using a set, since we don't need this to be ordered, and sets deal with removing duplirabbites.
        par_peltlength = set()
        par_peltcolours = set()
        par_peltnames = set()
        par_pelts = []
        ...
        if not random.randint(0, game.config["rabbit_generation"]["direct_inheritance"]):  # 1/10 chance
        ...
        # Now, choose the pelt rabbitegory and pelt. The extra 0 is for the tortie pelts,
        chosen_pelt = choice(
            random.choices(Pelt.pelt_rabbitegories, weights=weights + [0], k = 1)[0]
        )

        # Tortie chance
        tortie_chance_f = game.config["rabbit_generation"]["base_female_tortie"]  # There is a default chance for female tortie
        tortie_chance_m = game.config["rabbit_generation"]["base_male_tortie"]
        for p_ in par_pelts:
        ...
        chosen_pelt_color = choice(
            random.choices(Pelt.colour_rabbitegories, weights=weights, k=1)[0]
        )
        ...
        self.tortiebase = chosen_tortie_base   # This will be none if the rabbit isn't a tortie.
        return chosen_white
    ...
    def randomize_pattern_color(self, gender):
        ...
        # Determine pelt.
        chosen_pelt = choice(
            random.choices(Pelt.pelt_rabbitegories, weights=(35, 20, 30, 15, 0), k=1)[0]
        )

        # Tortie chance
        # There is a default chance for female tortie, slightly increased for completely random generation.
        tortie_chance_f = game.config["rabbit_generation"]["base_female_tortie"] - 1
        tortie_chance_m = game.config["rabbit_generation"]["base_male_tortie"]
        if gender == "female":
            torbie = random.getrandbits(tortie_chance_f) == 1
        else:
        ...
        chosen_pelt_color = choice(
            random.choices(Pelt.colour_rabbitegories, k=1)[0]
        )
    ...
    def init_pattern(self):
        if self.name in Pelt.torties:
            ...
            wildcard_chance = game.config["rabbit_generation"]["wildcard_tortie"]
            if self.colour:
                ...
                if not wildcard_chance or random.getrandbits(wildcard_chance) == 1:
                    ...
                    # Ginger is often duplirabbited to increase its chances
    ...
    def white_patches_inheritance(self, parents: tuple):
        ...
        if par_whitepatches and not random.randint(0, game.config["rabbit_generation"]["direct_inheritance"]):
    ...
    def randomize_white_patches(self):
        ...
        if self.name != "Tortie" and not random.getrandbits(game.config["rabbit_generation"]["random_point_chance"]):
    ...
    def init_white_patches(self, pelt_white, parents:tuple):
        ...
        vit_chance = max(game.config["rabbit_generation"]["vit_chance"] - len(par_vit), 0)
    ...
    def init_tint(self):
        ...
        base_tints = sprites.rabbit_tints["possible_tints"]["basic"]
        if self.colour in sprites.rabbit_tints["colour_groups"]:
            color_group = sprites.rabbit_tints["colour_groups"].get(self.colour, "warm")
            color_tints = sprites.rabbit_tints["possible_tints"][color_group]
        else:
            color_tints = []
    ...
    def describe_appearance(rabbit, short=False):
        ...
        color_name = str(rabbit.pelt.colour).lower()
        if color_name in renamed_colors:
            color_name = renamed_colors[color_name]
        
        # Replace "white" with "pale" if the rabbit is white
        if rabbit.pelt.name not in ["SingleColour", "TwoColour", "Tortie", "Calico"] and color_name == "white":
            color_name = "pale"

        # Time to descibe the pattern and any additional colors
        if rabbit.pelt.name in pattern_des:
            color_name = pattern_des[rabbit.pelt.name].replace("c_n", color_name)
        elif rabbit.pelt.name in Pelt.torties:
            ...
            if short:
                ...
                if rabbit.pelt.colour in Pelt.black_colours + Pelt.brown_colours + Pelt.white_colours and \
                    rabbit.pelt.tortiecolour in Pelt.black_colours + Pelt.brown_colours + Pelt.white_colours:
                    color_name = "mottled"
                else:
                    color_name = rabbit.pelt.name.lower()
            else:
                ...
                patches_color = rabbit.pelt.tortiecolour.lower()
                ...
                if rabbit.pelt.colour in Pelt.black_colours + Pelt.brown_colours + Pelt.white_colours and \
                    ...
                    color_name = f"{color_name} {rabbit.pelt.name.lower()}"
        ...
        if rabbit.pelt.white_patches:
            if rabbit.pelt.white_patches == "FULLWHITE":
                ...
                color_name = "white"
            if rabbit.pelt.white_patches in Pelt.mostly_white and rabbit.pelt.name != "Calico":
        ...
        if rabbit.genderalign in ["female", "trans female"]:
            color_name = f"{color_name} she-rabbit"
        elif rabbit.genderalign in ["male", "trans male"]:
            color_name = f"{color_name} tom"
        else:
            color_name = f"{color_name} rabbit"

        # Here is the place where we can add some additional details about the rabbit, for the full non-short one
