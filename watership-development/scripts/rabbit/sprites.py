...
class Sprites():
    ...
    def load_tints(self):
        try:
            with open("sprites/dicts/tint.json", 'r') as read_file:
                self.rabbit_tints = ujson.loads(read_file.read())
        except:
    ...
    def load_all(self):
        ...
        if isinstance(self.size, int):
            ...
            self.size = 50 # default, what base warrengen uses
            print(f"lineart.png is not 3x7, falling back to {self.size}")
            print(f"if you are a modder, please update scripts/rabbit/sprites.py and do a search for 'if width / 3 == height / 7:'")
        ...
        for x in [
            ...
            'medrabbitherbs', 'lineartdf', 'lightingnew', 'fademask',
            'fadeinle', 'fadedarkforest'
    ...
    def load_scars(self):
        ...
        for a, i in enumerate(
                ["BURNBELLY", "BEAKCHEEK", "BEAKLOWER", "BURNRUMP", "RABBITBITE", "RATBITE",
        ...
        for a, i in enumerate([
            ...
            self.make_group('medrabbitherbs', (a, 0), f'acc_herbs{i}')
        for a, i in enumerate([
            "BLUEBELLS", "NETTLE", "POPPY", "LAVENDER", "HERBS", "PETALS"]):
            self.make_group('medrabbitherbs', (a, 1), f'acc_herbs{i}')
        for a, i in enumerate([
            "OAK LEAVES", "MINT", "MAPLE SEED", "JUNIPER"]):
            self.make_group('medrabbitherbs', (a, 3), f'acc_herbs{i}')
        self.make_group('medrabbitherbs', (5, 2), 'acc_herbsDRY HERBS')

        for a, i in enumerate([
            "RED FEATHERS", "BLUE FEATHERS", "JAY FEATHERS", "MOTH WINGS", "CICADA WINGS"]):
            self.make_group('medrabbitherbs', (a, 2), f'acc_wild{i}')
