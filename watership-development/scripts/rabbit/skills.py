...
class SkillPath(Enum):
    ...
    STAR = (
        "curious about the old stories",
        "connection to Frith",
        "deep bond with Frith",
        "unshakable link to Frith",
    )
    DARK = (
        "interested in the lightless lands",
        "Dark affinity",
        "deep bond with the dark",
        "unshakable link with the dark",
    )
    OMEN = (
...
class Skill():
    ...
    short_strings = {
        ...
        SkillPath.HARVESTER: "harvesting",
        ...
        SkillPath.DARK: "lightless lands"
    }
...
class RabbitSkills:
    ...
    influence_flags = {
        ...
        SkillPath.HARVESTER: SkillTypeFlag.STRONG | SkillTypeFlag.AGILE | SkillTypeFlag.OBSERVANT,
    ...
    def __repr__(self) -> str:
        return f"<RabbitSkills: Primary: |{self.primary}|, Secondary: |{self.secondary}|, Hidden: |{self.hidden}|>"
    
    @staticmethod
    def generate_new_rabbitskills(status, months, hidden_skill:HiddenSkillEnum=None):
        '''Generates a new skill'''
        new_skill = RabbitSkills()
        
        new_skill.hidden = hidden_skill       

        #TODO: Make this nicer
        if status == "newborn" or months <= 0:
            pass
    ...
    def rusasirah_influence(self, rusasirah):
        """
        this function handles rusasirah influence on the rabbit's skill
        :param the_rabbit: the rabbit object
        :param rusasirah: the rusasirah's rabbit object
        """
        ...
        rusasirah_tags = RabbitSkills.influence_flags[rusasirah.skills.primary.path] if rusasirah.skills.primary else None

        can_primary = bool(
            RabbitSkills.influence_flags[self.primary.path] & rusasirah_tags) if self.primary and rusasirah_tags else False
        can_secondary = bool(
            RabbitSkills.influence_flags[self.secondary.path] & rusasirah_tags) if self.secondary and rusasirah_tags else False
        ...
        return (rusasirah.ID, path, amount_effect)

    def progress_skill(self, the_rabbit):
        """
        this function should be run every month for every rabbit to progress their skills accordingly
        :param the_rabbit: the rabbit object for affected rabbit
        """
        
        if the_rabbit.status == 'newborn' or the_rabbit.months <= 0:
            return
        
        # Give a primary is there isn't one already, and the rabbit is older than one month. 
        if not self.primary:
            parents = [the_rabbit.fetch_rabbit(i) for i in [the_rabbit.parent1, the_rabbit.parent2] + the_rabbit.adoptive_parents if 
                    type(the_rabbit) == type(the_rabbit.fetch_rabbit(i))]
        ...
        if not (the_rabbit.outside or the_rabbit.exiled):
            ...
            if the_rabbit.status == 'kit':
                ...
                # if the the_rabbit has skills, check if they get any points this month
                ...
                # Check if they get any points this month
                if not int(random.random() * 4):
                ...
                chance = max(1, 160 - the_rabbit.months)
                if not int(random.random() * chance):  # chance increases as the_rabbit ages
    ...
    @staticmethod
    def get_skills_from_old(old_skill, status, months):
        """Generates a RabbitSkill object"""
        
        new_skill = RabbitSkills()
        
        conversion = {
            "strong connection to Inlé": (SkillPath.STAR, 2), 
            ...
            "good owsla": (SkillPath.MEDIATOR, 1),
            "great owsla": (SkillPath.MEDIATOR, 2),
            "excellent owsla": (SkillPath.MEDIATOR, 3),
            "smart": (SkillPath.CLEVER, 1),
            "very smart": (SkillPath.CLEVER, 2),
            "extremely smart": (SkillPath.CLEVER, 3),
            "good harvester": (SkillPath.HARVESTER, 1),
            "great harvester": (SkillPath.HARVESTER, 2),
            "fantastic harvester": (SkillPath.HARVESTER, 3),
        ...
        if old_skill in conversion:
            ...
            new_skill = RabbitSkills.generate_new_rabbitskills(status, months)
