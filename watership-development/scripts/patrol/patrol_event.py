...
class PatrolEvent:
    ...
    @property
    def other_warren(self) -> bool:
        """Return boolian indirabbiting if any outcome has any reputation effect"""
        for out in self.success_outcomes + self.fail_outcomes + \
             self.antag_fail_outcomes + self.antag_success_outcomes:
            if out.other_warren_rep is not None:
                return True
...
"tags": list - list of strings ONLY FOR THINGS THAT ARE NEEDED IN THE PATROL FILTERING PROCESS. Right now, only "new_rabbit"
"min_rabbits": int
"max_rabbits": int
...
"success_outcomes : [
    {
        ...
        "dead_rabbits": [various, strs, telling, which, rabbits, should die]
        "injury: {
            "rabbits": [list of rabbits]
            "injury": [list of injury],
            "scars" [list of scars]
        }
        "lost_rabbits": [list, of rabbits]
        "history_text": {
            "reg_death": "",
            "lead_death: "",
            "scar": "",
        ...
        :: outcome a new_rabbit outcome.
        "new_rabbit": [
            [ details for new_rabbit block, includeing "meet" or "join" ],
            [  ],
            [  ]
        ]
        ...
        "other_warren_rep": int (-2, 1, 0, 1, 2) IE, gain, neutral or lose
        "relationship" [
            {
                :: at least two ::
                rabbit_from: str
...
OLD
    ...
    border patrols - "border", "other_warren", "reputation",

    med patrols - "med_rabbit", "herb", "random_herbs", "many_herbs#"

    un-used for now - "npc", "gone_rabbit"
...
-- history text -- 
    ...
    History text[1] is death text for normal rabbits
...
-- PATROL ABBREVIATIONS --
    ...
    Random rabbit - r_r
    Patrol leader - p_l
    Stat Cat - s_c (this is the rabbit with relevant skills/traits for the situation)
    Apprentice 1 - app1
    Apprentice 2 - app2
    Apprentice 3 - app3 
    Apprentice 4 - app4 
...
-- PATROL ID GUIDELINES --
    ...
    Training - train
    Med Cat - med
    If no specific type, pick one bc they gotta be rabbitegorized somewhere.  Make dupes in each type if you feel like 
    ...
    other descriptors being used to make identifirabbition and sorting easier. 

-- RELATIONSHIP CONSTRAINT:
    This is an optional constraint, if you use this, all rabbits in the patrol has to have these relation.
    If there are multiple 'tags', all 'tags' will be be used for the filtering.

    general:
    "sibling", "mates"
    "parent/child" -> patrol leader is parent, random rabbit is child
    "child/parent" -> patrol leader is child, random rabbit is parent

    'thresholds':
    for a 'threshold' tag, you only have to add the value type and then a number.
    !ALL! relationship values of each rabbit to each other has to have these values or higher
    "romantic_NUMBER",
    "platonic_NUMBER",
    "dislike_NUMBER",
    "comfortable_NUMBER",
...
-- TAG INFO: --
    ...
    "death" (r_r dies), "disaster" (all die), "multi_deaths" (2-4 rabbits die)
    If you have more than one, it takes the first one in this order.
    same for: "gone" (r_r leaves the clan), "disaster_gone" (all leave the clan), "multi_gone" (2-4 rabbits leave the clan)

    #!FOR INJURIES, SEE CONDITIONS LIST FOR TAGGING
    Tag all injury patrols that should give a scar with "scar" to ensure that classic mode will still scar the rabbit.
    ...
    If you want to injure all the rabbits in the patrol, tag with "injure_all"
    This will work with any condition whether they are an illness, injury, or perm condition
    If you want to ensure that a rabbit cannot die from the condition, tag with "non_lethal"
    ...
    Right now, nonlethal shock is auto applied to all rabbits present when another rabbit dies. This may change in the future.

    ! To tag injuries/illnesses on rabbits joining, you MUST use "new_rabbit_injury"
    ...
    - RELATIONSHIP TAGS -
        ...
        # whole Clan gains relationship towards r_r - "clan_to_r_c"
        ...
        # patrol gains relationship towards r_r - "patrol_to_r_c"
        ...
        "sacrificial" is for fail outcomes where a rabbit valiantly sacrifices themselves for the Clan 
        (such as the single rabbit big dog patrol) this will give the tagged for group ("clan_to_r_c", "patrol_to_r_c", etc) 
        a big boost to respect and trust in that rabbit even though they failed (if the rabbit survives lol) Other tagged for values 
        ...
        "no_change_fail_rep" is for when rep should not change when a new_rabbit patrol fails
