...
class CeremonyScreen(Screens):
    ...
    def screen_switches(self):
        ...
        if self.the_rabbit.status == 'chief_rabbit':
            self.header = pygame_gui.elements.UITextBox(str(self.the_rabbit.name) + '\'s Leadership Ceremony',
                                                        scale(pygame.Rect((200, 180), (1200, -1))),
                                                        object_id=get_text_box_theme(), manager=MANAGER)
        else:
            self.header = pygame_gui.elements.UITextBox(str(self.the_rabbit.name) + ' has no ceremonies to view.',
                                                        scale(pygame.Rect((200, 180), (1200, -1))),
                                                        object_id=get_text_box_theme(), manager=MANAGER)
        if self.the_rabbit.status == 'chief_rabbit' and not self.the_rabbit.dead:
            self.life_text = History.get_lead_ceremony(self.the_rabbit)
