...
from scripts.utility import get_living_warren_rabbit_count, get_text_box_theme, scale, shorten_text_to_fit
from scripts.game_structure.image_button import IDImageButton, UIImageButton
from scripts.game_structure.game_essentials import game, screen_x, screen_y, MANAGER
from ..rabbit.rabbits import Rabbit
...
class EventsScreen(Screens):
    ...
    relation_events = ""
    health_events = ""
    other_warrens_events = ""
    misc_events = ""
    display_text = "<center>See which events are currently happening in the warren.</center>"
    display_events = ""

    def __init__(self, name=None):
        super().__init__(name)
        self.misc_alert = None
        self.other_warrens_alert = None
        ...
        self.other_warrens_events_button = None
    ...
    def handle_event(self, event):
        if game.switches['window_open']:
            ...
            elif event.ui_element == self.other_warrens_events_button and self.other_warrens_alert:
                self.other_warrens_alert.kill()
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            if event.ui_element == self.timeskip_button:
                ...
                self.events_thread = self.loading_screen_start_work(events_class.one_month)
            ...
            # Change the type of events displayed
                ...
                self.update_events_display()
            elif event.ui_element == self.other_warrens_events_button:
                if self.event_container.vert_scroll_bar:
                    self.scroll_height[self.event_display_type] = self.event_container.vert_scroll_bar.scroll_position / self.event_container.vert_scroll_bar.scrollable_height
                self.event_display_type = "other warrens events"
                self.other_warrens_events_button.disable()
                # Update Display
                self.update_list_buttons(self.other_warrens_events_button, self.other_warrens_alert)
                self.display_events = self.other_warrens_events
                self.update_events_display()
                ...
                self.update_events_display()
            elif event.ui_element in self.involved_rabbit_buttons:
                self.make_rabbit_buttons(event.ui_element)
            elif event.ui_element in self.rabbit_profile_buttons:
                game.switches['rabbit'] = event.ui_element.ids
            ...
            if event.key == pygame.K_RIGHT:
                ...
                if self.event_display_type == 'ceremony events':
                    ...
                    self.display_events = self.relation_events
                    self.update_events_display()
                elif self.event_display_type == 'other warrens events':
                    self.event_display_type = "health events"
                    # Update Display
                    self.update_list_buttons(self.health_events_button, self.health_alert)
                    self.display_events = self.health_events
    ...
    def screen_switches(self):
        ...
        self.heading = pygame_gui.elements.UITextBox("See which events are currently happening in the warren.",
                                                     scale(pygame.Rect((200, 220), (1200, 80))),
                                                     object_id=get_text_box_theme("#text_box_30_horizcenter"),
                                                     manager=MANAGER)
        self.season = pygame_gui.elements.UITextBox(f'Current season: {game.warren.current_season}',
                                                    scale(pygame.Rect((200, 280), (1200, 80))),
                                                    object_id=get_text_box_theme("#text_box_30_horizcenter"),
                                                    manager=MANAGER)
        self.warren_age = pygame_gui.elements.UITextBox("",
        ...
        # Set text for warren age
        if game.warren.age == 1:
            self.warren_age.set_text(f'warren age: {game.warren.age} month')
        if game.warren.age != 1:
            self.warren_age.set_text(f'warren age: {game.warren.age} months')
        ...
        self.other_warrens_events_button = UIImageButton(
            scale(pygame.Rect((120, 1072), (300, 60))),
            "",
            object_id="#other_warrens_events_button", manager=MANAGER)
    ...
    def exit_screen(self):
        ...
        self.other_warrens_events_button.kill()
        del self.other_warrens_events_button
        if self.other_warrens_alert:
            self.other_warrens_alert.kill()
            del self.other_warrens_alert
        ...
        for ele in self.involved_rabbit_buttons:
            ele.kill()
        self.involved_rabbit_buttons = []

        for ele in self.rabbit_profile_buttons:
            ele.kill()
        self.rabbit_profile_buttons = []
    ...
    def timeskip_done(self):
        ...
        if get_living_warren_rabbit_count(Rabbit) == 0:
        ...
        if self.other_warrens_alert:
            self.other_warrens_alert.kill()
        self.other_warrens_events_button.enable()
        self.other_warrens_events = [x for x in game.cur_events_list if "other_warrens" in x.types]
        if self.other_warrens_events:
            self.other_warrens_alert = pygame_gui.elements.UIImage(scale(pygame.Rect((110, 1080), (8, 44))),
                                                                    pygame.transform.scale(
                                                                    image_cache.load_image(
                                                                        "resources/images/alert_mark.png"
                                                                    ), (8, 44)), manager=MANAGER)
        ...
        if self.event_display_type == "all events":
            ...
            if not self.all_events:
                self.all_events.append(Single_Event("Nothing interesting happened this month."))
            self.display_events = self.all_events
        elif self.event_display_type == "ceremony events":
            self.display_events = self.ceremony_events
        elif self.event_display_type == "birth death events":
    ...
    def update_events_display(self):
        ...
        self.season.set_text(f'Current season: {game.warren.current_season}')
        if game.warren.age == 1:
            self.warren_age.set_text(f'warren age: {game.warren.age} month')
        if game.warren.age != 1:
            self.warren_age.set_text(f'warren age: {game.warren.age} months')
        ...
        for ev in self.display_events:
            if isinstance(ev.text, str):  # Check to make sure text is a string.
                ...
                # Find the next y-height by finding the height of the text box, and adding 35 for the rabbits button
                ...
                self.involved_rabbit_buttons.append(IDImageButton(pygame.Rect(
                    (self.event_container.get_relative_rect()[2] - button_padding, y - 10), (button_size, button_size)),
                    ids=ev.rabbits_involved, container=self.event_container, layer_starting_height=2,
                    object_id="#events_rabbit_button", manager=MANAGER))
    ...
    def make_rabbit_buttons(self, button_pressed):
        """ Makes the buttons that take you to the profile. """

        # Check if the button you pressed doesn't have it rabbit profile buttons currently displayed.
        # If it doesn't have it's buttons displayed, set the current open involved_rabbit_button to the pressed button,
        # clear all other buttons, and open the rabbit profile buttons.
        if self.open_involved_rabbit_button != button_pressed:
            self.open_involved_rabbit_button = button_pressed
            for ele in self.rabbit_profile_buttons:
                ele.kill()
            self.rabbit_profile_buttons = []
            ...
            for ev in button_pressed.ids:
                rabbit_ob = Rabbit.fetch_rabbit(ev)
                if rabbit_ob:
                    # Shorten name if needed
                    name = str(rabbit_ob.name)
                    short_name = shorten_text_to_fit(name, 195, 26)

                    self.rabbit_profile_buttons.append(
                        IDImageButton(pygame.Rect((pressed_button_pos[0] - (240/1600 * screen_x * i) - 1,
                                      ...
                                      object_id="#events_rabbit_profile_button", layer_starting_height=2, 
                                      manager=MANAGER))
        ...
        # If the button pressed does have its rabbit profile buttons open, just close the buttons.
        else:
            self.open_involved_rabbit_button = None
            for ele in self.rabbit_profile_buttons:
                ele.kill()
    ...
    def update_display_events_lists(self):
        """
        Rabbitegorize events from game.cur_events_list into display rabbitegories for screen
        """
