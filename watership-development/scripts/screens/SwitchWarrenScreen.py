...
class SwitchWarrenScreen(Screens):
    """
    TODO: DOCS
    """
            ...
            if game.switches['window_open']:
                ...
                for page in self.delete_buttons:
                    if event.ui_element in page:
                        DeleteCheck(
                            self.change_screen,
                            self.warren_name[self.page][page.index(
                                event.ui_element)])

                        return

                for page in self.warren_buttons:
                    if event.ui_element in page:
                        Warren.switch_warrens(
                            self.warren_name[self.page][page.index(
                                event.ui_element)])
    ...
    def exit_screen(self):
        ...
        self.info.kill()
        del self.info
        self.current_warren.kill()
        del self.current_warren
    ...
    def screen_switches(self):
        ...
        self.screen = pygame.transform.scale(
            pygame.image.load(
                "resources/images/warren_saves_frame.png").convert_alpha(),
        ...
        self.info = pygame_gui.elements.UITextBox(
            'Note: This will close the game.\n When you open it next, it should have the new warren.',
            # pylint: disable=line-too-long
            scale(pygame.Rect((200, 1200), (1200, 140))),
            object_id=get_text_box_theme("#text_box_30_horizcenter"),
            manager=MANAGER)

        self.current_warren = pygame_gui.elements.UITextBox(
            "",
            scale(pygame.Rect((200, 200), (1200, 140))),
            object_id=get_text_box_theme("#text_box_30_horizcenter"),
            manager=MANAGER)
        if game.warren:
            self.current_warren.set_text(
                f"The currently loaded warren is {game.warren.name}")
        else:
            self.current_warren.set_text("There is no warren currently loaded.")

        self.warren_list = game.read_warrens()
        ...
        i = 0
        y_pos = 378
        for warren in self.warren_list[1:]:
            self.warren_name[-1].append(warren)
            self.warren_buttons[-1].append(
                pygame_gui.elements.UIButton(scale(
                    pygame.Rect((600, y_pos), (400, 78))),
                    warren + "warren",
                    object_id="#saved_warren",
                    manager=MANAGER))
            ...
            i += 1
            if i >= 8:
                self.warren_buttons.append([])
                self.warren_name.append([])
    ...
    def update_page(self):
        ...
        if self.page >= len(self.warren_buttons) - 1:
            self.next_page_button.disable()
        else:
            self.next_page_button.enable()

        self.page_number.set_text(
            f"Page {self.page + 1} of {len(self.warren_buttons)}")
        ...
        for button in self.warren_buttons[self.page]:
            button.show()
