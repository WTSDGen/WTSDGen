...
class ChooseAdoptiveParentScreen(Screens):
    ...
    def screen_switches(self):
        ...
        self.info = pygame_gui.elements.UITextBox(
            "If a rabbit is added as an adoptive parent, they will be displayed on the family page and considered a full relative. "
        ...
        self.help_button = UIImageButton(scale(pygame.Rect(
                ...
                tool_tip_text=  "A rabbit's adoptive parents are set automatically when the rabbit is born. "
                                "Any rabbits that are mates with the parents at the time of birth are considered adoptive parents."
                                "<br><br>"
                                "To be a possible adoptive parent, the rabbit has to be 14 months older than the child.",
            )
    ...
    def change_adoptive_parent(self):
        ...
        if self.selected_rabbit.ID not in self.the_rabbit.adoptive_parents:
            self.the_rabbit.adoptive_parents.append(self.selected_rabbit.ID)
            self.the_rabbit.create_inheritance_new_rabbit()
            
        else:
            self.the_rabbit.adoptive_parents.remove(self.selected_rabbit.ID)
            self.the_rabbit.create_inheritance_new_rabbit()
            self.selected_rabbit.create_inheritance_new_rabbit()
    ...
    def update_birth_container(self):
        ...
        birth_parents = [Rabbit.fetch_rabbit(i) for i in [self.the_rabbit.parent1, self.the_rabbit.parent2] if
        ...
        if len(birth_parents) == 1:
            self.birth_parents_buttons["rabbit"] = UISpriteButton(
                    ...
                    rabbit_object=birth_parents[0], manager=MANAGER, 
            ...
            for i, _par in enumerate(birth_parents):
                self.birth_parents_buttons["rabbit" + str(i)] = UISpriteButton(
                    ...
                    rabbit_object=_par, manager=MANAGER, 
                    container=self.birth_container,
    ...
    def update_adoptive_parents_container(self):
        ...
        self.all_adoptive_parents = self.chunks([Rabbit.fetch_rabbit(i) for i in self.the_rabbit.adoptive_parents if
                                          isinstance(Rabbit.fetch_rabbit(i), Rabbit)], 30)
    ...
    def update_adoptive_parents_container_page(self):
        ...
        if self.all_adoptive_parents:
            display_rabbits = self.all_adoptive_parents[self.adoptive_page]
        else:
            display_rabbits = []
    ...
    def update_potential_mates_container_page(self):
        ...
        if self.all_potential_parents:
            display_rabbits = self.all_potential_parents[self.potential_parents_page]
        else:
            display_rabbits = []
    ...
    def update_current_rabbit_info(self, reset_selected_rabbit=True):
        ...
        self.current_rabbit_elements["heading"] = pygame_gui.elements.UITextBox(
            "Choose adoptive parents for " + str(self.the_rabbit.name),
            scale(pygame.Rect((300, 50), (1000, 80))),
            object_id=get_text_box_theme("#text_box_34_horizcenter"))

        self.current_rabbit_elements["image"] = pygame_gui.elements.UIImage(scale(pygame.Rect((1200, 300), (300, 300))),
                                                                         pygame.transform.scale(
        ...
        if reset_selected_rabbit:
            ...
            for x in self.the_rabbit.adoptive_parents:
                parent_ob = Rabbit.fetch_rabbit(x)
                if not parent_ob.faded:
                    self.selected_rabbit = Rabbit.fetch_rabbit(parent_ob)
            self.update_selected_rabbit()
    ...
    def draw_tab_button(self):
        ...
        if self.the_rabbit.adoptive_parents:
        ...
        if self.the_rabbit.parent1 or self.the_rabbit.parent2:
    ...
    def update_toggle_button(self):
        ...
        elif self.selected_rabbit.ID in self.the_rabbit.adoptive_parents:
    ...
    def update_selected_rabbit(self):
        ...
        self.selected_rabbit_elements["image"] = pygame_gui.elements.UIImage(scale(pygame.Rect((100, 300), (300, 300))),
                                                                  pygame.transform.scale(
    ...
    def on_use(self):
        ...
        self.loading_screen_on_use(self.work_thread, self.update_after_change, (700, 600))

    def get_valid_adoptive_parents(self):
        """Get a list of valid parents for the current rabbit"""
        valid_parents = [inter_rabbit for inter_rabbit in Rabbit.all_rabbits_list if
                         not (inter_rabbit.dead or inter_rabbit.outside or inter_rabbit.exiled) and  # Adoptive parents cant be dead or outside
                         inter_rabbit.ID != self.the_rabbit.ID and # Can't be your own adoptive parent
                         inter_rabbit.months - self.the_rabbit.months >= 14 and # Adoptive parent must be at least 14 months older. -> own child can't adopt you
                         inter_rabbit.ID not in self.the_rabbit.mate and # Can't set your mate your adoptive parent. 
                         inter_rabbit.ID not in self.the_rabbit.get_parents() and # Adoptive parents can't already be their parent
                         self.not_related_to_mate(inter_rabbit) and # quick fix TODO: change / remove later
                         (not self.mates_current_parents or self.is_parent_mate(self.the_rabbit, inter_rabbit)) and #Toggle for only mates of current parents
                         (not self.unrelated_only or inter_rabbit.ID not in self.the_rabbit.get_relatives())] #Toggle for only not-closely-related. 
    ...
    def not_related_to_mate(self, possible_parent) -> bool:
        ...
        """
        if len(self.the_rabbit.mate) > 0:
            for mate_id in self.the_rabbit.mate:
                mate = Rabbit.fetch_rabbit(mate_id)
    ...
    def get_previous_next_rabbit(self):
        ...
        for check_rabbit in Rabbit.all_rabbits_list:
            ...
            if self.next_rabbit == 0 and check_rabbit.ID != self.the_rabbit.ID and check_rabbit.dead == self.the_rabbit.dead and \
                    ...
                    check_rabbit.df == self.the_rabbit.df:
    ...
    @staticmethod
    def is_parent_mate(the_rabbit:Rabbit, other_rabbit:Rabbit):
        """Checks to see if other_rabbit is the mate of any of current parents """
        for x in the_rabbit.get_parents():
            ob = Rabbit.fetch_rabbit(x)
            if not isinstance(ob, Rabbit):
                continue
            if other_rabbit.ID in ob.mate:
                return True
