...
class FamilyTreeScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.root_rabbit = None
        self.family_tree = None
        self.center_rabbit_frame = None
        self.root_rabbit_frame = None
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.back_button:
                ...
                game.switches['root_rabbit'] = None
                ...
                if isinstance(Rabbit.fetch_rabbit(self.previous_rabbit), Rabbit):
                    ...
                    game.switches['root_rabbit'] = Rabbit.all_rabbits[self.previous_rabbit]
                    self.exit_screen()
                ...
                if isinstance(Rabbit.fetch_rabbit(self.next_rabbit), Rabbit):
                    ...
                    game.switches['root_rabbit'] = Rabbit.all_rabbits[self.next_rabbit]
                    self.exit_screen()
                    self.screen_switches()
                else:
                ...
                self.group_page_number += 1
                self.handle_relation_groups()
            elif event.ui_element == self.rabbit_elements["center_rabbit_image"]:
                self.change_screen('profile screen')
                game.switches['root_rabbit'] = None
            elif event.ui_element in self.relation_elements.values() or self.rabbit_elements.values():
                try:
                    id = event.ui_element.return_rabbit_id()
                    if Rabbit.fetch_rabbit(id).faded:
                        return
    ...
    def screen_switches(self):
        ...
        self.previous_rabbit_button = UIImageButton(scale(pygame.Rect((50, 50), (306, 60))), "",
                                                 object_id="#previous_rabbit_button", manager=MANAGER)
        self.next_rabbit_button = UIImageButton(scale(pygame.Rect((1244, 50), (306, 60))), "",
                                             object_id="#next_rabbit_button", manager=MANAGER)
        ...
        if not game.switches['root_rabbit']:
            game.switches['root_rabbit'] = Rabbit.all_rabbits[game.switches['rabbit']]
        self.root_rabbit_frame = pygame_gui.elements.UIImage(scale(pygame.Rect((129, 950), (452, 340))),
                                                          pygame.transform.scale(
                                                              image_cache.load_image(
                                                                  "resources/images/familytree_bigrabbitbox.png").convert_alpha(),
                                                              (452, 340)), manager=MANAGER)
        self.rabbit_elements["root_rabbit_image"] = UISpriteButton(scale(pygame.Rect((462, 1151), (100, 100))),
                                                             game.switches['root_rabbit'].sprite,
                                                             rabbit_id=game.switches['root_rabbit'].ID,
                                                             manager=MANAGER,
                                                             tool_tip_text=f'Started viewing tree at {game.switches["root_rabbit"].name}')

        self.root_rabbit_frame.disable()

        self.center_rabbit_frame = pygame_gui.elements.UIImage(scale(pygame.Rect((0, 0), (160, 180))),
                                                            pygame.transform.scale(
                                                                image_cache.load_image(
                                                                    "resources/images/familytree_smallrabbitbox.png").convert_alpha(),
                                                                (160, 180)),
                                                            manager=MANAGER,
                                                            container=self.family_tree)
        self.center_rabbit_frame.disable()
        self.group_page_number = 1
        # self.family_setup()
    ...
    def create_family_tree(self):
        ...
        self.rabbit_elements["screen_title"] = pygame_gui.elements.UITextBox(f"{self.the_rabbit.name}'s Family Tree",
                                                                          scale(
                                                                              pygame.Rect((300, 50),
                                                                                          (1000, 100))),
        ...
        self.parents = self.the_rabbit.inheritance.get_parents()
        self.mates = self.the_rabbit.inheritance.get_mates()
        self.kits = self.the_rabbit.inheritance.get_children()
        self.kits_mates = self.the_rabbit.inheritance.get_kits_mates()
        self.siblings = self.the_rabbit.inheritance.get_siblings()
        self.siblings_mates = self.the_rabbit.inheritance.get_siblings_mates()
        self.siblings_kits = self.the_rabbit.inheritance.get_siblings_kits()
        self.parents_siblings = self.the_rabbit.inheritance.get_parents_siblings()
        self.cousins = self.the_rabbit.inheritance.get_cousins()
        self.grandparents = self.the_rabbit.inheritance.get_grand_parents()
        self.grandkits = self.the_rabbit.inheritance.get_grand_kits()
        ...
        self.rabbit_elements["the_rabbit_image"] = UISpriteButton(scale(pygame.Rect((150, 969), (300, 300))),
                                                            self.the_rabbit.sprite,
                                                            rabbit_id=self.the_rabbit.ID,
                                                            manager=MANAGER)
        name = str(self.the_rabbit.name)
        short_name = shorten_text_to_fit(name, 260, 22)
        self.rabbit_elements["viewing_rabbit_text"] = pygame_gui.elements.UITextBox(f"Viewing {short_name}'s Lineage",
                                                                              scale(
                                                                                  pygame.Rect((150, 1282), (300, 150))),
                                                                              object_id=get_text_box_theme(
                                                                                  "#text_box_22_horizcenter_spacing_95"),
                                                                              manager=MANAGER, )
        self.center_rabbit_frame = pygame_gui.elements.UIImage(scale(pygame.Rect((x_pos, y_pos), (160, 180))),
                                                            pygame.transform.scale(
                                                                image_cache.load_image(
                                                                    "resources/images/familytree_smallrabbitbox.png").convert_alpha(),
                                                                (160, 180)),
                                                            ...
                                                            container=self.family_tree)
        self.center_rabbit_frame.disable()
        self.rabbit_elements['center_rabbit_image'] = UISpriteButton(scale(pygame.Rect((x_pos + 30, y_pos + 20), (100, 100))),
                                                               self.the_rabbit.sprite,
                                                               rabbit_id=self.the_rabbit.ID,
                                                               manager=MANAGER,
                                                               container=self.family_tree)
        ...
        self.rabbit_elements["center_rabbit_name"] = pygame_gui.elements.ui_label.UILabel(scale(pygame.Rect((10 + x_pos, 90 + y_pos), (145, 100))), short_name, object_id="#text_box_22_horizcenter", manager=MANAGER, container=self.family_tree)
    ...
    def handle_relation_groups(self):
        ...
        if not self.current_group:
            self.relation_elements["no_rabbits_notice"] = pygame_gui.elements.UITextBox("None",
                                                                                     scale(
                                                                                         pygame.Rect(
                                                                                             (550, 1080),
                                                                                             (900, 60))),
        ...
        if _current_group:
            display_rabbits = _current_group[self.group_page_number - 1]
        else:
            display_rabbits = []
        ...
        pos_y = 0
        i = 0
        for kitty in display_rabbits:
            _kitty = Rabbit.fetch_rabbit(kitty)
            info_text = f"{str(_kitty.name)}"
            additional_info = self.the_rabbit.inheritance.get_rabbit_info(kitty)
    ...
    def exit_screen(self):
        ...
        self.root_rabbit_frame.kill()
        del self.root_rabbit_frame
