...
class UISpriteButton():
    ...
    def __init__(self, relative_rect, sprite, rabbit_id=None, visible=1, rabbit_object=None, starting_height=1,
        ...
        self.image.disable()
        # The transparent button. This a subclass that UIButton that also hold the rabbit_id.
        self.button = CatButton(relative_rect, visible=visible, rabbit_id=rabbit_id, rabbit_object=rabbit_object,
                                starting_height=starting_height, manager=manager, tool_tip_text=tool_tip_text,
                                container=container)

    def return_rabbit_id(self):
        return self.button.return_rabbit_id()

    def return_rabbit_object(self):
        return self.button.return_rabbit_object()
    ...
    '''This is to simplify event handling. Rather that writing 
            'if event.ui_element = rabbit_sprite_object.button'
            you can treat is as any other single pygame UI element and write:
            'if event.ui_element = rabbit_sprite_object. '''
...
class CatButton(pygame_gui.elements.UIButton):
    ...
    def __init__(self, relative_rect, rabbit_id=None, visible=True, rabbit_object=None, starting_height=1, manager=None, tool_tip_text=None,
                 container=None):
        self.rabbit_id = rabbit_id
        self.rabbit_object = rabbit_object
        super().__init__(relative_rect, "", object_id="#rabbit_button", visible=visible,
                         starting_height=starting_height, manager=manager, tool_tip_text=tool_tip_text, container=container)

    def return_rabbit_id(self):
        return self.rabbit_id

    def return_rabbit_object(self):
        return self.rabbit_object
