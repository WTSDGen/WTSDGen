...
class SaveCheck(UIWindow):
    def __init__(self, last_screen, isMainMenu, mm_btn):
        ...
        self.warren_name = "Undefined"
        if game.warren:
            self.warren_name = f"{game.warren.name}"
        ...
        self.save_button_saving_state = pygame_gui.elements.UIImage(
            ...
            pygame.transform.scale(
                image_cache.load_image(
                    'resources/images/save_warren_saving.png'),
                (228, 60)),
            container=self)
...
class DeleteCheck(UIWindow):
    def __init__(self, reloadscreen, warren_name):
        ...
        self.set_blocking(True)
        game.switches['window_open'] = True
        self.warren_name = warren_name
        self.reloadscreen = reloadscreen

        self.delete_check_message = UITextBoxTweaked(
            f"Do you wish to delete {str(self.warren_name)}? This is permanent and cannot be undone.",
            scale(pygame.Rect((40, 40), (520, -1))),
            line_spacing=1,
            object_id="#text_box_30_horizcenter",
            container=self
    ...
    def process_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.delete_it_button:
                game.switches['window_open'] = False
                print("delete")
                rempath = get_save_dir() + "/" + self.warren_name
...
class GameOver(UIWindow):
    def __init__(self, last_screen):
        ...
        self.set_blocking(True)
        game.switches['window_open'] = True
        self.warren_name = str(game.warren.name)
        self.last_screen = last_screen
        self.game_over_message = UITextBoxTweaked(
            f"{self.warren_name} has died out. For now, this is where their story ends. Perhaps it's time to tell a new "
            f"tale?",
            scale(pygame.Rect((40, 40), (520, -1))),
            line_spacing=1,
            object_id="",
...
class ChangeRabbitName(UIWindow):
    ...
    def __init__(self, rabbit):
        super().__init__(scale(pygame.Rect((600, 430), (800, 370))),
                         ...
                         object_id='#change_rabbit_name_window',
                         resizable=False)
        game.switches['window_open'] = True
        self.the_rabbit = rabbit
        self.back_button = UIImageButton(
        ...
        self.specsuffic_hidden = self.the_rabbit.name.specsuffix_hidden

        self.heading = pygame_gui.elements.UITextBox(f"-Change {self.the_rabbit.name}'s Name-",
                                                     scale(pygame.Rect(
                                                         (0, 20), (800, 80))),
                                                     object_id="#text_box_30_horizcenter",
                                                     manager=MANAGER,
        ...
        self.prefix_entry_box = pygame_gui.elements.UITextEntryLine(
            ...
            initial_text=self.the_rabbit.name.prefix,
            manager=MANAGER,
            container=self)
        ...
        self.toggle_spec_block_on = UIImageButton(scale(pygame.Rect((405 + x_pos, 160 + y_pos), (68, 68))), "",
                                                  ...
                                                  tool_tip_text=f"Remove the rabbit's special suffix",
                                                  manager=MANAGER,
                                                  container=self)

        self.toggle_spec_block_off = UIImageButton(scale(pygame.Rect((405 + x_pos, 160 + y_pos), (68, 68))), "",
                                                   object_id="#checked_checkbox",
                                                   tool_tip_text="Re-enable the rabbit's special suffix", manager=MANAGER,
                                                   container=self)


        if self.the_rabbit.name.status in self.the_rabbit.name.names_dict["special_suffixes"]:
            self.suffix_entry_box = pygame_gui.elements.UITextEntryLine(
                scale(pygame.Rect((318 + x_pos, 100 + y_pos), (240, 60))),
                placeholder_text=self.the_rabbit.name.names_dict["special_suffixes"]
                [self.the_rabbit.name.status], manager=MANAGER,
                container=self)
            if not self.the_rabbit.name.specsuffix_hidden:
                ...
                # handle when the suffix entry box shows up
            ...
            # handle when the suffix entry box shows up, & where

            self.suffix_entry_box = pygame_gui.elements.UITextEntryLine(
                scale(pygame.Rect((318 + x_pos, 100 + y_pos), (240, 60))),
                initial_text="", manager=MANAGER,
                container=self)
            self.suffix_entry_box.disable()
            self.suffix_entry_box.hide()

            # now the prefix box & randomize button -- we have to hide them first & make them
            # show up again

            self.prefix_entry_box.hide()
            self.random_prefix.hide()

            # change positions
            self.prefix_entry_box = pygame_gui.elements.UITextEntryLine(
                scale(pygame.Rect((168 + x_pos, 100 + y_pos), (240, 60))),
                initial_text=self.the_rabbit.name.prefix,
                manager=MANAGER,
                container=self)
            self.random_prefix = UIImageButton(scale(pygame.Rect((415 + x_pos, 97 + y_pos), (68, 68))), "",
                                           object_id="#random_dice_button",
                                           manager=MANAGER,
                                           container=self,
                                           tool_tip_text='Randomize the prefix')
            
            # show em again. theres probably a better way to do this but idk it
            self.prefix_entry_box.show()
            self.random_prefix.show()
        self.set_blocking(True)
    ...
    def process_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.done_button:
                old_name = str(self.the_rabbit.name)

                self.the_rabbit.specsuffix_hidden = self.specsuffic_hidden
                self.the_rabbit.name.specsuffix_hidden = self.specsuffic_hidden
                ...
                if sub(r'[^A-Za-z0-9 ]+', '', self.prefix_entry_box.get_text()) != '':
                    self.the_rabbit.name.prefix = sub(
                ...
                # by a special suffix.
                if self.the_rabbit.name.status not in self.the_rabbit.name.names_dict["special_suffixes"] or \
                        self.the_rabbit.name.specsuffix_hidden:
                    self.name_changed.show()

                if old_name != str(self.the_rabbit.name):
                    self.name_changed.show()
                    self.heading.set_text(
                        f"-Change {self.the_rabbit.name}'s Name-")
                else:
                    self.name_changed.hide()

            elif event.ui_element == self.random_prefix:
                self.prefix_entry_box.set_text(Name(self.the_rabbit.status,
                                                    None,
                                                    self.the_rabbit.pelt.colour,
                                                    self.the_rabbit.pelt.eye_colour,
                                                    self.the_rabbit.pelt.name,
                                                    self.the_rabbit.pelt.tortiepattern).prefix)
...
class SpecifyRabbitGender(UIWindow):
    """This window allows the user to specify the rabbit's gender"""

    def __init__(self, rabbit):
        super().__init__(scale(pygame.Rect((600, 430), (800, 370))),
                         window_display_title='Change Rabbit Gender',
                         object_id='#change_rabbit_gender_window',
                         resizable=False)
        game.switches['window_open'] = True
        self.the_rabbit = rabbit
        self.back_button = UIImageButton(
        ...
        self.heading = pygame_gui.elements.UITextBox(f"-Change {self.the_rabbit.name}'s Gender-"
        ...
        self.gender_entry_box = pygame_gui.elements.UITextEntryLine(scale(pygame.Rect((200, 180), (400, 60))),
                                                                    placeholder_text=self.the_rabbit.genderalign,
                                                                    manager=MANAGER,
                                                                    container=self)
        self.set_blocking(True)
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.done_button:
                if sub(r'[^A-Za-z0-9 ]+', "", self.gender_entry_box.get_text()) != "":
                    self.the_rabbit.genderalign = sub(
...
class KillRabbit(UIWindow):
    ...
    def __init__(self, rabbit):
        ...
        self.heading = pygame_gui.elements.UITextBox(f"<b>-- How did this rabbit die? --</b>",
                                                     scale(pygame.Rect(
                                                         (20, 20), (860, 150))),
                                                     object_id="#text_box_30_horizcenter_spacing_95",
                                                     manager=MANAGER,
        ...
        if self.the_rabbit.status == 'chief rabbit':
            ...
            self.prompt = process_text(
                'This rabbit died when {PRONOUN/m_c/subject}...', rabbit_dict)
            self.initial = process_text(
                '{VERB/m_c/were/was} killed by a higher power.', rabbit_dict)

            self.all_lives_check.hide()
            self.life_text = pygame_gui.elements.UITextBox('Kill the chief rabbit',
                                                           scale(pygame.Rect(
                                                               (120, 295), (900, 80))),
                                                           object_id="#text_box_30_horizleft",
                                                           manager=MANAGER,
        ...
        elif History.get_death_or_scars(self.the_rabbit, death=True):
            # This should only occur for retired chief rabbits.

            self.prompt = process_text(
                'This rabbit died when {PRONOUN/m_c/subject}...', rabbit_dict)
            self.initial = process_text(
                '{VERB/m_c/were/was} killed by something unknowable to even Frith', rabbit_dict)
            self.all_lives_check.hide()
            ...
            self.initial = 'This rabbit was killed by a higher power.'
            self.prompt = None
            self.all_lives_check.hide()
            self.one_life_check.hide()
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.done_button:
                ...
                if self.the_rabbit.status == 'chief rabbit':
                    ...
                    if self.take_all:
                        game.warren.chief_rabbit_lives = 0
                    else:
                        game.warren.chief_rabbit_lives -= 1

                self.the_rabbit.die()
                self.history.add_death(self.the_rabbit, death_message)
                update_sprite(self.the_rabbit)
                game.switches['window_open'] = False
                game.all_screens['profile screen'].exit_screen()
                game.all_screens['profile screen'].screen_switches()
                self.kill()
...
class UpdateAvailablePopup(UIWindow):
    def __init__(self, last_screen, show_checkbox: bool = False):
        ...
        self.game_over_message = UITextBoxTweaked(
            f"<strong>Update to WarrenGen {latest_version_number}</strong>",
        ...
        if dynamic_changelog:
            ...
            for line in commits:
                ...
                if pr_number:
                    ...
                    info[2] += f" <a href='https://github.com/WarrenGenOfficial/warrengen/pull/{pr_number.group(1)}'>(link)</a>"
...
class ChangeRabbitToggles(UIWindow):
    ...
    def __init__(self, rabbit):
        ...
        self.text_2 = pygame_gui.elements.UITextBox("Prevent kittens", scale(pygame.Rect(110, 110, -1, 50)), 
                                                    object_id="#text_box_30_horizleft_pad_0_8",
                                                    container=self)
    ...
    def refresh_checkboxes(self):
        ...
        # Prevent Fading
        if self.the_rabbit == game.warren.instructor:
            box_type = "#checked_checkbox"
            tool_tip = "The afterlife guide can never fade."
        elif self.the_rabbit.prevent_fading:
            box_type = "#checked_checkbox"
            tool_tip = "Prevents rabbit from fading away after being dead for 202 months."
        else:
        ...
        #No Kits
        if self.the_rabbit.no_kittens:
            box_type = "#checked_checkbox"
            tool_tip = "Prevent the rabbit from adopting or having kittens."
        else:
            box_type = "#unchecked_checkbox"
            tool_tip = "Prevent the rabbit from adopting or having kittens."
        
        self.checkboxes["prevent_kittens"] = UIImageButton(scale(pygame.Rect(45, 100, 68, 68)), "",
                                                        container=self,
                                                        object_id=box_type,
                                                        tool_tip_text=tool_tip)
        
        #No Retire
        if self.the_rabbit.no_retire:
            box_type = "#checked_checkbox"
            tool_tip = "Allow rabbit to retiring automatically."
        else:
            box_type = "#unchecked_checkbox"
            tool_tip = "Prevent rabbit from retiring automatically."
        ...
        #No mates
        if self.the_rabbit.no_mates:
            box_type = "#checked_checkbox"
            tool_tip = "Prevent rabbit from automatically taking a mate, breaking up, or having romantic interactions with non-mates."
        else:
            box_type = "#unchecked_checkbox"
            tool_tip = "Prevent rabbit from automatically taking a mate, breaking up, or having romantic interactions with non-mates."
    ...
    def process_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.back_button:
                ...
                self.the_rabbit.prevent_fading = not self.the_rabbit.prevent_fading
                self.refresh_checkboxes()
            elif event.ui_element == self.checkboxes["prevent_kittens"]:
                self.the_rabbit.no_kittens = not self.the_rabbit.no_kittens
                self.refresh_checkboxes()
            elif event.ui_element == self.checkboxes["prevent_retire"]:
                self.the_rabbit.no_retire = not self.the_rabbit.no_retire
                self.refresh_checkboxes()
            elif event.ui_element == self.checkboxes["prevent_mates"]:
                self.the_rabbit.no_mates = not self.the_rabbit.no_mates
                self.refresh_checkboxes()
