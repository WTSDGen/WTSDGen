...
class Single_Interaction():
    ...
    def __init__(self,
        ...
        if interactions:
            ...
            self.interactions = [f"This is a default interaction! ID: {id} with rabbits (m_c), (r_r)"]
        ...
        if reaction_random_rabbit:
            self.reaction_random_rabbit = reaction_random_rabbit
        else:
            self.reaction_random_rabbit = {}
...
class Group_Interaction():
    ...
    def __init__(self, 
        ...
        self.biome = biome if biome else ["Any"]
        self.season = season if season else ["Any"]
        self.rabbit_amount = rabbit_amount
...
def rel_fulfill_rel_constraints(relationship, constraint, interaction_id) -> bool:
    ...
    if "siblings" in constraint and not relationship.rabbit_from.is_sibling(relationship.rabbit_to):
        return False

    if "mates" in constraint and (relationship.rabbit_from.ID not in relationship.rabbit_to.mate or\
                                  relationship.rabbit_to.ID not in relationship.rabbit_from.mate ):
        return False

    if "not_mates" in constraint and (relationship.rabbit_from.ID in relationship.rabbit_to.mate or\
                                      relationship.rabbit_to.ID in relationship.rabbit_from.mate):
        return False

    if "parent/child" in constraint and not relationship.rabbit_from.is_parent(relationship.rabbit_to):
        return False

    if "child/parent" in constraint and not relationship.rabbit_to.is_parent(relationship.rabbit_from):
        return False
...
def rabbits_fulfill_single_interaction_constraints(main_rabbit, random_rabbit, interaction, game_mode) -> bool:
    """Check if the two rabbits fulfills the interaction constraints."""
    if len(interaction.main_status_constraint) >= 1:
        if main_rabbit.status not in interaction.main_status_constraint:
            return False

    if len(interaction.random_status_constraint) >= 1:
        if random_rabbit.status not in interaction.random_status_constraint:
            return False

    if len(interaction.main_trait_constraint) >= 1:
        if main_rabbit.personality.trait not in interaction.main_trait_constraint:
            return False

    if len(interaction.random_trait_constraint) >= 1:
        if random_rabbit.personality.trait not in interaction.random_trait_constraint:
            return False

    if len(interaction.main_skill_constraint) >= 1:
        if (main_rabbit.skills.primary.skill or main_rabbit.skills.secondary.skill) not in interaction.main_skill_constraint:
            return False

    if len(interaction.random_skill_constraint) >= 1:
        if (random_rabbit.skills.primary.skill or random_rabbit.skills.secondary.skill) not in interaction.random_skill_constraint:
            return False
        
    if len(interaction.backstory_constraint) >= 1:
        if "m_c" in interaction.backstory_constraint:
            if main_rabbit.backstory not in interaction.backstory_constraint["m_c"]:
                return False
        if "r_r" in interaction.backstory_constraint:
            if random_rabbit.backstory not in interaction.backstory_constraint["r_r"]:
                return False
    ...
    if len(interaction.has_injuries) >= 1:
        ...
        if "m_c" in interaction.has_injuries:
            injuries_in_needed = list(
                filter(lambda inj: inj in interaction.has_injuries["m_c"], main_rabbit.injuries.keys())
            )
            if len(injuries_in_needed) <= 0:
                return False
        if "r_r" in interaction.has_injuries:
            injuries_in_needed = list(
                filter(lambda inj: inj in interaction.has_injuries["r_r"], random_rabbit.injuries.keys())
            )
            if len(injuries_in_needed) <= 0:
                return False
...
def create_interaction(inter_list) -> list:
    ...
    for inter in inter_list:
        created_list.append(Single_Interaction(
            ...
            reaction_random_rabbit= inter["reaction_random_rabbit"] if "reaction_random_rabbit" in inter else None,
...
def create_group_interaction(inter_list) -> list:
    ...
    for inter in inter_list:
        created_list.append(Group_Interaction(
            ...
            biome=inter["biome"] if "biome" in inter else ["Any"],
            season=inter["season"] if "season" in inter else ["Any"],
            rabbit_amount=inter["rabbit_amount"] if "rabbit_amount" in inter else None,
            intensity=inter["intensity"] if "intensity" in inter else "medium",
            interactions=inter["interactions"] if "interactions" in inter else None,
            get_injuries=inter["get_injuries"] if "get_injuries" in inter else None,
            has_injuries=inter["has_injuries"] if "has_injuries" in inter else None,
