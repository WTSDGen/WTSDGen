...
class Relationship():
    ...
    def __init__(self, rabbit_from, rabbit_to, mates=False, family=False, romantic_love=0, platonic_like=0, dislike=0,
                 admiration=0, comfortable=0, jealousy=0, trust=0, log=None) -> None:
        self.history = History()
        self.rabbit_from = rabbit_from
        self.rabbit_to = rabbit_to
        self.mates = mates
        self.family = family
    ...
    def link_relationship(self):
        ...
        if self.rabbit_from.ID in self.rabbit_to.relationships:
            self.opposite_relationship = self.rabbit_to.relationships[self.rabbit_from.ID]
        else:
            # create relationship
            relation = Relationship(self.rabbit_to, self.rabbit_from)
            self.rabbit_to.relationships[self.rabbit_from.ID] = relation
    ...
    def start_interaction(self) -> None:
        ...
        if self.rabbit_from.dead or self.rabbit_from.outside or self.rabbit_from.exiled:
            return
        if self.rabbit_to.dead or self.rabbit_to.outside or self.rabbit_to.exiled:
            return

        # update relationship
        if self.rabbit_to.ID in self.rabbit_from.mate:
            self.mates = True
        ...
        if self.chosen_interaction.get_injuries and game_mode != 'classic':
            ...
            for abbreviations, injury_dict in self.chosen_interaction.get_injuries.items():
                ...
                injured_rabbit = self.rabbit_from
                if abbreviations != "m_c":
                    injured_rabbit = self.rabbit_to
                ...
                if possible_scar or possible_death:
                    for condition in injuries:
                        self.history.add_possible_history(injured_rabbit, condition, scar_text=possible_scar, death_text=possible_death)
        ...
        if self.rabbit_from.months == 1:
            self.log.append(interaction_str + f" - {self.rabbit_from.name} was {self.rabbit_from.months} month old")
        else:
            self.log.append(interaction_str + f" - {self.rabbit_from.name} was {self.rabbit_from.months} months old")
        ...
        game.cur_events_list.append(Single_Event(
            interaction_str, ["relation", "interaction"], [self.rabbit_to.ID, self.rabbit_from.ID]
        ))
    ...
    def adjust_interaction_string(self, string):
        ...
        rabbit_dict = {
            "m_c": (str(self.rabbit_from.name), choice(self.rabbit_from.pronouns)),
            "r_r": (str(self.rabbit_to.name), choice(self.rabbit_to.pronouns))
        }

        return process_text(string, rabbit_dict)
    ...
    def get_amount(self, in_de_crease: str, intensity: str) -> int:
        ...
        compatibility = get_personality_compatibility(self.rabbit_from, self.rabbit_to)
    ...
    def interaction_affect_relationships(self, in_de_crease: str, intensity: str, rel_type: str) -> None:
        ...
        rel_dict = self.chosen_interaction.reaction_random_rabbit
        if rel_dict:
    ...
    def positive_interaction(self) -> bool:
        ...
        comp = get_personality_compatibility(self.rabbit_from, self.rabbit_to)
        if comp is not None:
    ...
    def get_interaction_type(self, positive: bool) ->  str:
        ...
        mate_from_to = self.rabbit_from.is_potential_mate(self.rabbit_to, for_love_interest=True)
        mate_to_from = self.rabbit_to.is_potential_mate(self.rabbit_from, for_love_interest=True)
    ...
    def get_relevant_interactions(self, interactions : list, intensity : str, biome : str, season : str, game_mode : str) -> list:
        ...
        for interact in interactions:
            ...
            rabbits_fulfill_conditions = rabbits_fulfill_single_interaction_constraints(self.rabbit_from, self.rabbit_to, interact, game_mode)
            if not rabbits_fulfill_conditions:
                continue
