...
# earl_ans = ancestry.loc[ancestry['Relation'] == 'Earliest Ancestor', 'Rabbit 1'].iloc[0]
# ancestry['recorded_ind'] = 0    # Flag for saying rabbits whose data has been recorded in the tree
#
# incomp = [earl_ans]
# comp = []
#
...
# det = str(ancestry.loc[ancestry['Rabbit 1'] == earl_ans, 'Details'][0])
# g = ancestry.loc[ancestry['Rabbit 1'] == earl_ans, 'Gender'][0]
# sh = 'rect' if g == 'M' else 'ellipse'
# dot.node(earl_ans, earl_ans, tooltip = det, shape = sh)
# node_nm.append(earl_ans)
#
# ancestry.loc[ancestry['Rabbit 1'] == earl_ans, 'recorded_ind'] = 1
#
...
#         break
#     else:
#         temp['this_gen_ind'] = temp.apply(lambda x: 1 if x['Rabbit 2'] in incomp else 0, axis = 1)
#
#         # Spouse Relationship
#         this_gen = temp[(temp['this_gen_ind'] == 1) & (temp['Relation'] == 'Spouse')]
#         if len(this_gen) > 0:
#             for j in range(0, len(this_gen)):
#                 per1 = this_gen['Rabbit 1'].iloc[j]
#                 per2 = this_gen['Rabbit 2'].iloc[j]
...
#
#         comp.extend(incomp)
#         incomp = list(temp.loc[temp['this_gen_ind'] == 1, 'Rabbit 1'])
#         ancestry['recorded_ind'] = temp.apply(lambda x: 1 if (x['Rabbit 1'] in incomp) | (x['Rabbit 1'] in comp) else 0, axis = 1)
#
# dot.format = 'svg'
