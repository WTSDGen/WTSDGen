...
if not getattr(sys, 'frozen', False):
    ...
    if isMissing:
        if find_spec("thonny") is not None:
            print("""You are missing some requirements to run Watership!
                  Please press "Tools" -> "Manage Packages"
                  Once the menu opens, click the link below "Install from requirements file".
                  Then, select the file "requirements.txt" in the Watership folder.
                  """)
        else:
            print("""You are missing some requirements to run Watership!
        ...
        print("If you are still having issues, please ask for help in the ClanGen discord server: https://discord.gg/clangen As this is a mod of ClanGen, please check the thread in #Mod-Library.")
        sys.exit(1)
...
# Logging for file
timestr = time.strftime("%Y%m%d_%H%M%S")
log_file_name = get_log_dir() + f"/Watership_{timestr}.log"
...
if os.environ.get('CODESPACES'):
    ...
    print(
        ...
        + "/?autoconnect=true&reconnect=true&password=Watership&resize=scale")
    print("(use Watership in fullscreen mode for best results)")
    print('')
...
# Load game
from scripts.game_structure.load_rabbit import load_rabbits, version_convert
...
from scripts.warren import warren_class
...
def load_data():
    ...
    warren_list = game.read_warrens()
    if warren_list:
        game.switches['warren_list'] = warren_list
        try:
            load_rabbits()
            version_info = warren_class.load_warren()
            ...
            if not game.switches['error_message']:
                game.switches[
                    'error_message'] = 'There was an error loading the rabbits file!'
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            # Dont display if on the start screen or there is no warren.
